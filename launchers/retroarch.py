...
def _get_retroarch_installation_type() -> str:
    if run_command(f"flatpak list | grep {RETROARCH_FLATPAK_ID}").returncode == 0:
        return "flatpak"
    if run_command("which retroarch").returncode == 0:
        return "native"
    return ""


def detect_retroarch_installation() -> bool:
    return bool(_get_retroarch_installation_type())


def get_retroarch_config_path() -> str:
    if _get_retroarch_installation_type() == "flatpak":
...
def get_retroarch_cores_directory() -> str:
    ...
    # Flatpak RetroArch ships cores under /app/libretro inside the sandbox;
    # this path is passed through to the launched process even from the host.
    if _get_retroarch_installation_type() == "flatpak":
        return "/app/libretro"
...
def get_retroarch_command() -> str:
    t = _get_retroarch_installation_type()
    if t == "flatpak":
